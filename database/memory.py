...
logger = logging.getLogger("agent_news.memory")
...
def get_memories(user_id: str, query: str) -> list[str]:
    ...
    Returns a list of plain-text memory strings to be injected into context.
    Memories include user topic preferences, interests, and past reading patterns.
    """
    try:
        client = _get_client()
...
def save_memory(user_id: str, query: str, response: str) -> None:
    """Add the latest conversation turn to Mem0.

    Mem0 automatically extracts and stores user preferences, topic interests,
    and reading patterns from the exchange for future personalization.
    """
    try:
        client = _get_client()
        messages = [
